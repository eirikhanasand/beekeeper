...
export async function getPageOfLocalLogForContextNamespaceCache(page: number, context: string, namespace: string) {
    ...
    const logQueryFile = 'fetchLocalLogNoSearch.sql'
    const logCountQueryFile = 'fetchLocalLogCountNoSearch.sql'
    const logQueryParameters = [Number(page) || 1, resultsPerPage, namespace, context]
    const logCountQueryParameters = [namespace, context]
